terraform {
  ...
  backend "s3" {
    ...
    key                         = "deployments/healthcare-optimizer-for-medication-reco/terraform.tfstate"
    region                      = "us-east-1"
...
resource "digitalocean_project" "main" {
  name        = "ab-healthcare-optimizer-for-medication-reco"
  description = "AgentBridge: Healthcare Optimizer for Medication Reconciliation"
...
resource "digitalocean_vpc" "main" {
  name        = "ab-healthcare-optimizer-for-medication-reco-vpc"
  region      = var.region
  ip_range    = var.vpc_cidr
  description = "Isolated VPC for AgentBridge Healthcare Optimizer for Medication Reconciliation"
}
...
resource "digitalocean_ssh_key" "deploy" {
  ...
  name       = "ab-healthcare-optimizer-for-medication-reco-deploy"
...
resource "digitalocean_firewall" "main" {
  name = "ab-healthcare-optimizer-for-medication-reco-fw"
...
resource "digitalocean_droplet" "main" {
  ...
  tags = concat([
    "agentbridge",
    "blueprint-healthcare-optimizer-for-medication-reco",
    var.environment,
...
resource "digitalocean_volume" "data" {
  ...
  region                  = var.region
  name                    = "ab-healthcare-optimizer-for-medication-reco-data"
  size                    = var.additional_storage_gb
  initial_filesystem_type = "ext4"
  description             = "Persistent data for AgentBridge Healthcare Optimizer for Medication Reconciliation"

  tags = ["agentbridge", "blueprint-healthcare-optimizer-for-medication-reco", "data"]
}
...
resource "digitalocean_spaces_bucket" "artifacts" {
  ...
  name   = "ab-healthcare-optimizer-for-medication-reco-artifacts"
...
resource "digitalocean_loadbalancer" "main" {
  ...
  name   = "ab-healthcare-optimizer-for-medication-reco-lb"
  region = var.region
  vpc_uuid = digitalocean_vpc.main.id
...
resource "digitalocean_monitor_alert" "cpu_high" {
  ...
  description = "AgentBridge healthcare-optimizer-for-medication-reco: CPU > 85% for 5 min"
}
...
resource "digitalocean_monitor_alert" "memory_high" {
  ...
  enabled     = true
  entities    = [digitalocean_droplet.main.id]
  description = "AgentBridge healthcare-optimizer-for-medication-reco: Memory > 90% for 5 min"
}
...
resource "digitalocean_monitor_alert" "disk_high" {
  ...
  enabled     = true
  entities    = [digitalocean_droplet.main.id]
  description = "AgentBridge healthcare-optimizer-for-medication-reco: Disk > 85%"
}
